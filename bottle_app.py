...
def home():
    ...
    # Get collection names and parse language fields    
    for collection_name in db.list_collection_names():
        col = db[collection_name]
        doc = col.find_one()
        if doc:
            fields = [k for k in doc.keys() if k != '_id']
            languages.append({"collection": collection_name, "fields": fields})

    return template('index.tpl', languages=languages)
...
def post_selected_lesson():
    language = request.query.collection
    print(f"{language} is the language selected")
    collection = db[language]
    vocab = list(collection.find())
    shuffled_vocab = random.shuffle(vocab)
    quiz_questions = []
    for x in range (0,len(vocab)):
        quiz_questions[x+1] = (x, shuffled_vocab["french"], shuffled_vocab["english"])
